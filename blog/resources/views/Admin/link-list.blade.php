		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				...
				<tr class="text-c">
					<td>{{ $v->id }}</td>
					<td>{{ $v->name }}</td>
					<td><a href="HTTP://{{ $v->url }}" target="_blank">{{ $v->url }}</a></td>
					<td>{{ $v->text }}</td>
					<td>{{ $v->created_at }}</td>
					<td class="td-manage">
						<a style="text-decoration:none" class="ml-5" onClick="picture_edit('图库编辑','picture-add.html','10001')" href="/admin/link/{{ $v->id }}/edit" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> 
						<form action="/admin/link/{{ $v->id }}" method="post" style="display: inline-block;">
                		{{ csrf_field() }}
                		{{ method_field('DELETE') }}
                		<i ><input type="submit" class="Hui-iconfont" value="&#xe6e2;" onclick="return confrim("确定要删除吗")"></i>
...
function picture_stop(obj,id){
	layer.confirm('确认要下架吗？',function(index){
		$(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="picture_start(this,id)" href="javascript:;" name="status" title="发布"><i class="Hui-iconfont">&#xe603;</i></a>');
...
function picture_start(obj,id){
	layer.confirm('确认要发布吗？',function(index){
		$(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" onClick="picture_stop(this,id)" href="javascript:;" name="status" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a>');
		$(obj).parents("tr").find(".td-status").html('<span class="label label-success radius">已发布</span>');
		$(obj).remove();
		layer.msg('已发布!',{icon: 6,time:1000});
	});
